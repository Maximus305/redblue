...
export function useCloneGame(roomCode: string) {
  ...
  const [preloadedAiResponse, setPreloadedAiResponse] = useState<string | null>(null);
  ...
  // Preload AI response when the speaker decision screen appears
  useEffect(() => {
    const shouldPreload =
      ...
      me &&
      currentRound &&
      currentRound.speakerChoice === null &&
      !preloadedAiResponse &&
      !isPreloadingAi;

    if (!shouldPreload) return;

    const preloadAiResponse = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setPreloadedAiResponse(data.response);
        } else {
          setPreloadedAiResponse("I'd have to think about that one...");
        }
      } catch {
        setPreloadedAiResponse("I'd have to think about that one...");
      } finally {
        setIsPreloadingAi(false);
      }
    };

    preloadAiResponse();
  }, [room?.status?.phase, isSpeaker, me, currentRound, preloadedAiResponse, isPreloadingAi]);

  // Reset preloaded response when round changes
  useEffect(() => {
  ...
  const handleSpeakerChoice = useCallback(
    async (choice: 'AI' | 'Self') => {
      ...
      try {
        ...
        if (choice === 'AI') {
          // Use preloaded response if available, otherwise generate on the fly
          if (preloadedAiResponse) {
            updateData.aiAnswerPrivate = preloadedAiResponse;
          } else {
            // Fallback: generate response if preload didn't complete
            try {
              const response = await fetch('/api/generate-redblue-response', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    [me, currentRound, roomCode, preloadedAiResponse]
  );
